...
- テストの妥当性（アサーション十分か、エッジケースカバー）
- 問題が見つかった場合はステップ3に戻って修正する

### 5. テストカバレッジ調査

追加したテストのカバレッジを調査する:

- 実装した全関数・ロジックパスに対してテストが存在するか確認
- 未テストの重要な関数やエッジケースを特定
- 不足があればテストを追加し、再度 `cargo test` で全パスを確認
- テスト追加 → カバレッジ再調査のサイクルを、十分なカバレッジに達するまで繰り返す
